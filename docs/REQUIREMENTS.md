...
### v0.2 adapter addition (activity-pattern extraction)

*Added 2026-06-14, revised after a 3-panel council. The wrap/tail adapters surface the most-recent raw output line as the ACTIVITY label, which is noise when the wrapped tool is chatty. FR-56 lets the operator declare which lines are activity-relevant. **Build target: v0.2** (council recommendation; operator to confirm) — v0.1.0 just passed a clean release-gate, the feature is display-only and additive so deferral is structurally free, and "the regex plumbing is warm" is a velocity argument, not a correctness one (the FR-55 continuation-pressure shape). Spec captured + council-reviewed now. PENDING (§9).*
...
| Continuation contract: externalized per-tick continue/stop verdict (closed halt set) + yield accounting + abandonment / illegitimate-yield / false-halt-claim detector (FR-55, UC-11, US-11) | PENDING — v0.1.0 build; engine emits the per-tick verdict (+ `next_tick_due`, `monitoring_paused`, blocker records), integration scenario `continuation_contract` (stub-host honor / abandon / false-yield / false-halt-claim / crash-then-resume / scheduled-gap / blocked-then-clear) + dogfood audit (NFR-12) |
| Activity-pattern extraction for wrap/tail adapters (operator-declared relevance; display-only, not doneness) (FR-56, UC-12, US-12) | PENDING — spec'd + councilled 2026-06-14 (revised); **build target v0.2** (recommended); extends `heartbeat.py` adapter activity labeling (both adapters) + plan schema + `--check` compile-validation + ReDoS bounds (per-tick byte ceiling, complexity screen) |

---

*End of requirements v1.3. Functional/nonfunctional numbering is the stable reference for instructions, tests, and the eventual README. FR-34..50 + UC-9 are the v0.1.0 first-release scope (build status per-row in §9 — most VERIFIED as of v0.1.0; the cadence/Windows floor matrix stays PENDING); FR-51..55 + UC-10/11 are the v0.1.0 testing, UX, and autonomy-integrity additions (FR-55 PENDING); FR-56 + UC-12/US-12 is the adapter activity-extraction addition (PENDING, build target v0.2).*
